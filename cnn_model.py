...
def cnn(X, keep_prob):
    ...
    Wy = tf.Variable(np.array([[[0.1]], [[0.8]], [[0.1]]], dtype=np.float32))  # [ 0.15294118,  0.69803922,  0.14901961]
    y = fully_connected(X, 10, activation_fn=None)
    y = tf.expand_dims(y, 2)
    y = tf.nn.conv1d(y, Wy, 1, 'valid')
    y = tf.squeeze(y, [2])
